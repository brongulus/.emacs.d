...
(let* ((bg    "#ffffff")
       ...
       ;; Using special colors for mode-line backgrounds in
       ;; terminals, where I don't have box attributes
       (bg-mode-line (if (display-graphic-p) bg dim-0))
       (bg-mode-line-active (if (display-graphic-p) bg dim-1)))
  ...
  ;; (custom-set-faces
   ...
   `(mode-line-active ((t (:inherit mode-line :box (:color ,fg) :background ,bg-mode-line-active))))
   `(mode-line-inactive ((t (:inherit mode-line :foreground ,dim-3 :box (:color ,dim-1)))))
   ...
   `(info-menu-star ((t (:foreground ,error))))
   ;; mirror shr (html) headers
   `(info-title-1 ((t (:foreground ,string :bold t))))
   `(info-title-2 ((t (:foreground ,alt-fg :bold t))))
   `(info-title-3 ((t (:foreground ,fg :bold t))))
   `(info-title-4 ((t (:foreground ,string :underline t))))

   `(short-doc-heading ((t (:foreground ,string :bold t))))
   `(short-doc-heading ((t (:inherit default))))
